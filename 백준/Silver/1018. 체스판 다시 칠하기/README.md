...
## TIL
 <p>우, 하 값과 같으면 칠하고 카운트하는 방식으로 접근하였으나 이 경우 예제 4의 경우 32의 답을 출력함.</p>
 <p>대각선으로 나열된 값은 모두 같음을 이용하여 인덱스의 규칙을 이용하여 두가지 경우를 모두 카운트하여 최소값을 출력하는 방식으로 정답 도출.</p>
